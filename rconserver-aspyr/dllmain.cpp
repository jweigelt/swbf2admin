...
DWORD WINAPI Run(LPVOID module)
{
	...
	if (patchesApplied)
		Logger.log(LogLevel_INFO, "RconServer_64 loaded; patches applied; spawn timer %gs.",
				   bf2server_get_spawnvalue());
	else
	{
		Logger.log(LogLevel_ERROR, "RconServer_64 patch installation failed.");
		return 0;
	}
...
BOOL WINAPI DllMain(HINSTANCE hModule, DWORD dwReason, IN LPVOID)
{
	...
	{
		// Battlefront.exe loads Battlefront2.dll after injection; initialize on the worker.
